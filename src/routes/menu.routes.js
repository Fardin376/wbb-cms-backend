...
const checkDuplicateTitle = async (req, res, next) => {
  try {
    ...
    if (req.params.id) {
      if ((existingMenuEn && existingMenuEn._id.toString() !== req.params.id) ||
...
router.patch('/update-menu/:id', verifyToken, isAdminUser, validateMenuInput, checkDuplicateTitle, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ message: 'Menu Item updated successfully!', item: updatedMenu });
  } catch (err) {
...
router.get('/check-title', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error checking title uniqueness' });
  }
});
